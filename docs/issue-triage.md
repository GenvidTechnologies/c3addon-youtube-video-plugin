...
> `question`, `wontfix`) plus `youtube-port`, with **no** `type:`/`priority/`/`area:`
> scheme.
>
> The commands below use **GitHub Issues via the `gh` CLI**. Section headings must
...
The project-specific `youtube-port` label marks issues that are part of the
GCore→YouTube player port; it is **not** a category and coexists with one of the
...
- `youtube-port` — the GCore→YouTube port marker; orthogonal to category.
...
role — `needsInfoLabel` in the `bugTracker` block is set to it.)
...
Issue #1 is the `[Epic]` umbrella for the YouTube port; sub-tasks #2–#12 hang off
it. Prefer keeping that structure (task-list under a `Depends on` heading) over
re-splitting.
