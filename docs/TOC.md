...
- [decisions/0007-captions-map-retire-subtitle-aces.md](decisions/0007-captions-map-retire-subtitle-aces.md) — captions: keep and enrich `video-subtitles`/`SetSubtitles`/`Subtitles` (wires `cc_lang_pref` alongside `cc_load_policy`, both build-time only); retire the 9 GCore-era side-loaded/enumeration ACEs with no YouTube equivalent; live switching via the unofficial captions module deferred to a future issue (issue #6, epic #1).
- [decisions/0008-playback-rate-and-metadata-aces.md](decisions/0008-playback-rate-and-metadata-aces.md) — playback rate (pass-through `SetPlaybackRate`, authoritative `GetPlaybackRate`, indexed `GetAvailablePlaybackRate`/`Count`) and metadata (`GetVideoTitle` via unofficial `getVideoData()`, `GetPlayerUrl`, polled `GetVideoLoadedFraction`) ACEs; playlist load/navigation split out and deferred to a follow-up issue (issue #12, epic #1).
- [decisions/0009-gate-docs-drift-against-ace-surface.md](decisions/0009-gate-docs-drift-against-ace-surface.md) — `scripts/validate-docs.mjs` gates `docs/usage.md` only (README/TOC carry no checkable ACE content) via §8-table + backticked-PascalCase resolution against the ACE surface (Check A), plus an `en-US.json`-desc-vs-`plugin.ts`-default regression guard (Check B) so the enable-chrome class of drift (issue #30, PR #33) can't recur silently; closes issue #31.
